...
- [ ] **현재 워커 실행 이력 테이블 구조 분석**
  - [ ] AdminWorkersPage 컴포넌트에서 에러 메시지 표시 부분 확인
  - [ ] 에러 메시지 데이터 구조 및 표시 방식 파악
  - [ ] 현재 테이블 row 클릭 이벤트 처리 상태 확인

- [ ] **에러 메시지 상세 팝업 Modal 구현**
  - [ ] ErrorDetailModal 컴포넌트 생성
  - [ ] 전체 에러 메시지 텍스트 표시 (줄바꿈 포함)
  - [ ] 에러 발생 시간, 워커 종류 등 추가 정보 표시
  - [ ] JSON 형태의 에러 데이터인 경우 포맷팅 표시

- [ ] **복사 기능 구현**
  - [ ] Copy to Clipboard 버튼 추가
  - [ ] navigator.clipboard API 활용
  - [ ] 복사 성공 알림 (토스트 메시지)
  - [ ] 복사 실패 시 fallback 처리

- [ ] **테이블 row 클릭 이벤트 연동**
  - [ ] 에러 메시지가 있는 row에만 클릭 가능한 스타일 적용
  - [ ] 클릭 시 해당 에러 정보로 모달 오픈
  - [ ] 마우스 커서 변경 (pointer) 및 호버 효과

- [ ] **사용자 경험 개선**
  - [ ] 긴 에러 메시지도 읽기 쉽게 표시
  - [ ] 스크롤 가능한 텍스트 영역 제공
  - [ ] 모달 크기 조정 및 반응형 대응
  - [ ] 키보드 단축키 지원 (Ctrl+C/Cmd+C)

## Progress Status
- Current Progress: TASK_067 계획 완료 - 워커 실행 이력 에러 메시지 상세 팝업 및 복사 기능 구현 계획
- Next Task: TASK_067 실행 대기 - 사용자 ACT 명령 대기 중
- Last Update: 2025-06-19
- Automatic Check Status: PASS
